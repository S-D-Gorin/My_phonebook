...
def main():
    ...
    if read_notes():  # проверка наличия записей
        choice_option()  # функция выбора дальнейших действий
    else:
...
def read_notes():
    ...
    with open('db.TXT', mode='r', encoding='utf-8') as db:
        ...
        for line in db.readlines():  # каждая итерация цикла создает запись из БД во вложеном списке
            phonebook.append(line.split())
        # print(phonebook)
        return phonebook

def write_notes(data, mode):
    """Функция записи в БД в аркументах передается:
    1. что нужно записать
    2. метод записи: "W" для перезаписи всего файла или "A" его дополнения"""
...
def edit():
    ...
    if id > counter_lines():  # сравнение введенных данных и количиства записей в БД
        ...
        my_notes[id-1][0] = id  # переприсвоение номера записи

        write_notes('', 'w')  # очистка БД для перезаписи всех, в том числе и отредактированных данных из ОЗУ в БД
